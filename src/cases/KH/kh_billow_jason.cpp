...
double perturb_eig;             // Initial eigenfunction perturbation
...
char file_names_ordered[num_h_saves][20] 
    = {"Ubar.bin","Vbar.bin","Wbar.bin","RHObar.bin",
    "Urms.bin","Vrms.bin","Wrms.bin","RHOrms.bin",
    "UVp.bin","UWp.bin","VWp.bin",
    "URHOp.bin","VRHOp.bin","WRHOp.bin",
    "Dbar.bin","XiBar.bin",
    };
char file_temp_names_ordered[num_h_saves][20]  
    = {"Ubar_temp.bin","Vbar_temp.bin","Wbar_temp.bin","RHObar_temp.bin",
    "Urms_temp.bin","Vrms_temp.bin","Wrms_temp.bin","RHOrms_temp.bin",
    "UVp_temp.bin","UWp_temp.bin","VWp_temp.bin",
    "URHOp_temp.bin","VRHOp_temp.bin","WRHOp_temp.bin",
...
char slice_names_XZ[num_XZslices][20] = {"u_slice","v_slice","w_slice","rho_slice","D_slice"};
char slice_names_XY[num_XYslices][20] = {"u_slice","v_slice","w_slice","rho_slice","D_slice"};
    ...
    public:
        ...
        void init_vels(DTArray & u, DTArray & v, DTArray & w) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                // Add eigenfunctions with an amplitude equal to perturb
                u *= perturb_eig;
                w *= perturb_eig;
        ...
        void analysis(double time, DTArray & u, DTArray & v, DTArray & w,
            ...
            if ((time - slice_next_plot) > -1e-6) {
                ...
                write_slice_XZ(*tracers[RHO],slice_temp_file_XZ[RHO_SLICE],0);
                write_slice_XZ(*diss,slice_temp_file_XZ[D_SLICE],0);
        
                write_slice_XY(u,slice_temp_file_XY[U_SLICE],int(double(Nz)/2.0 - 1));
                write_slice_XY(v,slice_temp_file_XY[V_SLICE],int(double(Nz)/2.0 - 1));
                write_slice_XY(w,slice_temp_file_XY[W_SLICE],int(double(Nz)/2.0 - 1));
                write_slice_XY(*tracers[RHO],slice_temp_file_XY[RHO_SLICE],int(double(Nz)/2.0 - 1));
                write_slice_XY(*diss,slice_temp_file_XY[D_SLICE],int(double(Nz)/2.0 - 1));
            ...
            if ((time - havg_next_plot) > -1e-6) {
                ...
                // write_havg_variables(u,temp_file[U]);
                // write_havg_variables(*tracers[RHO],temp_file[RHO]);
                write_havg_variables(*diss,temp_file[D]);
                compute_xi(*diss, *tracers[RHO], gradient_op, grid_type, Nx, Ny, Nz, kappa_rho);
                write_havg_variables(*diss,temp_file[Xi]);

                write_havg_ReStress(u,v,w,*tracers[RHO],temp_file);
        ...
        void write_slice_XY(DTArray & q,MPI_File q_file, int Iz = 0){
            ...
            double *xy_data_buffer = new double[Nz*Ny/num_procs];
...
int main(int argc, char ** argv) {
    ...
    add_option("perturb_eig",&perturb_eig,"Initial perturbation in velocity");

...
double TOP_TEMP;               // Bottom Gradient
double TEMP_F_SCALE;           // IBM scale
...
class userControl : public BaseCase {
    public:
        ...
        void do_mapping(DTArray & xg, DTArray & yg, DTArray & zg) {
            ...
            // Define topography
            topo = hill_height*(1 + cos(M_PI*xx(ii)/Lx))/2;
        ...
        */
        ...
        void forcing(double t, const DTArray & u, DTArray & u_f,
            ...
            zgrid = alloc_array(Nx,Ny,Nz);
            *zgrid = MinZ + 0.5*Lz*(1+zz(kk)) + 0.5*(1-zz(kk))*topo(ii);
            ...
            *tracers_f[T] = TEMP_F_SCALE*(TOP_TEMP - *tracers[T])*exp(-pow((*zgrid-Lz)/(Lz/Nz*5),2));
            (*tracers_f[T])(all,all,0) = 0;    // Bottom
            (*tracers_f[T])(all,all,Nz-1) = 0;  // Top
...
int main(int argc, char ** argv) {
    ...
    add_option("TOP_TEMP",&TOP_TEMP,0.0,"Top T Value");
    add_option("TEMP_F_SCALE",&TEMP_F_SCALE,1.0,"IBM Forcing Timescale");
